...
#By default, manual control is off
manual = 0
...
Inte = DoubleVar()
Deri = DoubleVar()
scaleP = Scale(root, variable = Prop, from_=0, to=100000, length = 1000, resolution = 1, orient=HORIZONTAL)
scaleP.pack(anchor = CENTER)
scaleI = Scale(root, variable = Inte, from_=0, to=100000, length = 1000, resolution = 0.1, orient=HORIZONTAL)
scaleI.pack(anchor = CENTER)
scaleD = Scale(root, variable = Deri, from_=0, to=100000, length = 1000, resolution = 0.1, orient=HORIZONTAL)
...
def train_neural_network(data, layers, neurons_per_layer, no_outputs):
    ...
    while(1):
        ...
        if move == 0:
            ...
            #output_list.append([PID, time.time()-init_time, img2]) #Store Network output
##            if error == -1: #Turn Left
##                PWMvalueR = 500000
##                PWMvalueL = 300000
...
##                PWMvalueL = 400000
    ...
    camera.close()
    #np.save("Result_of_1l_21n_BalRan90000_Run_on_"+getTime(), output_list) #Save Ouput
    print('Output Saved')
